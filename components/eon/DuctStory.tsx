...
export default function DuctStory(){
 ...
 const [chapter,setChapter]=useState(0),[light,setLight]=useState(false);
 ...
 useEffect(()=>{
  ...
  const update=()=>{raf=0;const p=clamp((window.scrollY-top)/Math.max(travel,1));controller.current?.update(p,window.scrollY<top+travel+window.innerHeight&&window.scrollY+window.innerHeight>top);const next=storyState(p).chapter;if(next!==lastChapter.current){lastChapter.current=next;setChapter(next);}};
 ...
 return <section ref={section} className={`duct-story ${lighter?'light-story':''}`} aria-label="Inside the air">
  <div className="story-stage" data-chapter={chapter}>
   <div className="story-copy"><p className="eyebrow">EON Coatings / Inside the air</p><h1 className={chapter===0?'':'sr-only'}>A closer<br/>look at <br/>your air.</h1>{chapter!==0&&<h2>{chapters[chapter].title}</h2>}<p className="story-description">{chapters[chapter].copy}</p><Link prefetch={false} className="text-link" href="/contact-us/">Book a free consultation <span aria-hidden="true">↗</span></Link></div>
  ...
  <noscript><style>{`.duct-story{height:auto}.story-stage{position:relative}.static-sequence{display:block}.chapter-nav,.story-cue,.light-toggle{display:none}`}</style></noscript><div className="static-sequence">{chapters.slice(1).map((c,i)=><section key={c.id}><img src={`/model/${i===0?'dirty':i===1?'cleaning':i===2?'clean':'reassembled'}.webp`} alt="Illustrative cutaway of the same duct" width="1200" height="1000"/><div><p className="eyebrow">0{i+2} / {c.label}</p><h2>{c.title}</h2><p>{c.copy}</p></div></section>)}</div>
 </section>;
}

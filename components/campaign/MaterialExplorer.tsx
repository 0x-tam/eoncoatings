...
const surfaceTraces = [
 {title:'Spill residue',point:[940,675],copy:'A dried ring tells the story of a small spill. Cleaning and protective treatment depend on the stone and the liquid.'},
 {title:'Touch residue',point:[1000,670],copy:'Everyday handling can transfer skin oils and fine dirt. High-touch surfaces need suitable cleaning and care.'},
 {title:'Pores & particles',point:[945,728],copy:'Fine particles can lodge in natural openings. These pores are part of the stone, not proof of microbial growth.'}
];
const details={
...
export default function MaterialExplorer(){
 ...
 const tracePosition=(point:number[])=>{
  const zoom=scale*3.6;
  const x=Math.max(box.w-1672*zoom,Math.min(0,box.w*(box.w>600?.7:.5)-930*zoom));
  const y=Math.max(box.h-941*zoom,Math.min(0,box.h*.5-681*zoom));
  const overlayLeft=box.w<=600?0:box.w*(box.w<=1100?.42:.36);
 ...
 return <section className="room-hero" data-material={active||'overview'} data-settled={settled} aria-label="Explore EON care in a real room">
  <div className="room-image-field" ref={field}>
   ...
   {active&&<div className="room-overlays">
    {settled&&<div className="material-observation">{active==='air'?'DUST / MOISTURE / MICROBIAL GROWTH':active==='fabric'?'FIBER DETAIL / Woven texture & embedded particles':'SURFACE DETAIL / The traces of everyday life'}</div>}
    ...
    {active==='stone'&&settled&&<div className="surface-traces" aria-label="Explore surface traces">
     {surfaceTraces.map((item,i)=><button key={item.title} className="trace-point" aria-label={`${String(i+1).padStart(2,'0')} ${item.title}`} style={tracePosition(item.point)} aria-pressed={trace===i} onClick={()=>setTrace(i)}><b>0{i+1}</b><span>{item.title}</span></button>)}
     <div className="trace-card" aria-live="polite"><strong>{surfaceTraces[trace].title}</strong><p>{surfaceTraces[trace].copy}</p><small>Illustrative everyday traces, not a surface test.</small></div>
    </div>}
    {active!=='stone'&&<p className="inspection-caption">{d!.caption}</p>}
   </div>}

...
export default function SpacesGallery(){
 ...
 useEffect(()=>{if(paused||!inView)return;const timer=setInterval(()=>{if(!document.hidden)setSelected(i=>(i+1)%gallery.length)},3000);return()=>clearInterval(timer)},[paused,inView,selected]);
 ...
 return <div className="spaces-photo-gallery" ref={root}>
  ...
  <div className="spaces-photo-controls" role="group" aria-label="Browse spaces">{gallery.map((item,i)=><button type="button" key={item.id} aria-pressed={selected===i} aria-controls="spaces-photo-stage" onClick={()=>setSelected(i)}>{item.label}</button>)}</div>
  <button type="button" className="spaces-rotation-toggle" aria-label={paused?'Resume automatic slideshow':'Pause automatic slideshow'} aria-pressed={paused} onClick={()=>setPaused(v=>!v)}>{paused?<Play size={18} aria-hidden="true"/>:<Pause size={18} aria-hidden="true"/>}</button>
 </div>
}

import {seoCopy,articleTitles} from '@/lib/eon/seo-copy';
import {articleMarkup} from '@/lib/eon/article-markup';
import {siteUrl} from '@/lib/eon/metadata';
import {contentPath} from '@/lib/eon/urls';
...
export async function generateMetadata({params,searchParams}:{params:Promise<{slug:string[]}>;searchParams:Promise<Record<string,string|undefined>>}):Promise<Metadata>{
 ...
 const path=contentPath(slug.join('/')),record=lookup(path);
 ...
 const title=copy?.[0]||(service?seoCopy[service.slug]?.[0]:undefined)||(article?articleTitles[article.slug]:undefined)||service?.title||(sector?`${sectorName} Cleaning & Surface Protection`:undefined)||(archiveName?`${cleanText(archiveName)} Articles${pageNumber?` — Page ${pageNumber}`:''}`:undefined)||cleanText(record?.title||'Page Not Found');
 const description=copy?.[1]||(service?seoCopy[service.slug]?.[1]:undefined)||service?.short||(sector?`AC cleaning, fabric care and protective coatings for ${sectorName.toLowerCase()} facilities in Abu Dhabi and the UAE. Discuss access, materials and service planning.`:undefined)||(archiveName?`Browse Eon Coatings articles about ${cleanText(archiveName)}${pageNumber?`, page ${pageNumber}`:''}. Practical guidance on cleaning and protective coatings.`:undefined)||(record?cleanText(record.excerptHtml||record.contentText):defaultDescription);
 return pageMetadata({title,description,path:`/${path}/`,article:article?{publishedTime:article.date,modifiedTime:article.modified}:undefined,noindex:(!copy&&!service&&!record&&!archiveName)||kind==='tag'||kind==='author'||kind==='category'||Boolean(query.q||query.category||query.enquiry||query.qa)});
}
export default async function ContentPage({params,searchParams}:{params:Promise<{slug:string[]}>;searchParams:Promise<{q?:string;category?:string;limit?:string;space?:string}>}){
 const {slug}=await params;const search=await searchParams;const path=contentPath(slug.join('/'));
...
const r=lookup(path);const service=services.find(s=>path===`services/${s.slug}`);
 if(path==='media')return <main id="main"><PageHeading label="Eon Coatings media" title="News and practical guidance." description="Company updates, practical guidance and our complete article archive."/><MediaIndex articles={posts.map(({id,title,date,url,categories,contentText,featuredMedia})=>({id,title,date,url,categories,contentText,featuredMedia}))} initialQuery={search.q} initialCategory={search.category} initialLimit={Math.max(9,Number(search.limit)||9)}/><Consultation/></main>;

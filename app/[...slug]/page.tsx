...
import ServiceRange from '@/components/campaign/ServiceRange';
...
const r=lookup(path);const service=services.find(s=>path===`services/${s.slug}`);
 ...
 if(path==='service')return <main id="main" className="life-services"><PageHeading label="Eon Coatings / Services" title="Cleaning and coating services." description="For homes, offices, clinics, schools and hotels."/><ServiceRange/><section className="section service-faq"><div><p className="eyebrow">Surface protection</p><h2>Common service questions.</h2><p>Explore specialist cleaning, material care and protective coatings for your space.</p></div><Faqs/></section><section className="section service-choice-help"><p>Not sure which service you need?</p><Link href="/service-finder/" className="button">Find my service <span aria-hidden="true">›</span></Link></section></main>;
